...
/**
 * 获取 Mint 账户的 Program ID
 * @param connection 
 * @param mintAddress 
 * @returns 
 */
export async function getMintAccountProgramId(
  connection: Connection,
...
/**
 * 获取多个 Mint 账户的 Program ID
 * @param connection 
 * @param mintAddresses 
 * @returns 
 */
export async function getMultiMintAccountProgramId(
  connection: Connection,
  mintAddresses: PublicKey[]
): Promise<PublicKey[]> {
  const programIds: PublicKey[] = [];
  
  const accountInfos = await connection.getMultipleAccountsInfo(mintAddresses);
  if (!accountInfos || accountInfos.length !== mintAddresses.length) {
    throw new Error(`Some mint accounts not found: ${mintAddresses.map(mint => mint.toBase58()).join(", ")}`);
  }
  for (const accountInfo of accountInfos) {
    if (!accountInfo) {
      throw new Error(`Mint account not found: ${mintAddresses[accountInfos.indexOf(accountInfo)].toBase58()}`);
    }
